<template>
    <div id="toBaku" class="tabcontent">
        <form id="to_baku_form" :action="checkout_route" method="GET">
            <input type="hidden" :value="total_price ? total_price : location_price" class="last_price">
            <input type="hidden" name="type" value="2">
            <input type="hidden" name="transfer_type" value="to">
            ...
            <div class="where_and_from_wrapper">
                <div class="destination__car">
                    ...
                    <div class="custom-select" style="width: 100%">
                        <select id="test" @change="setLocation">
            ...
            <div class="select__car___date__wrapper">
                ...
                <div class="select__car__date__to">
                    ...
                    <div class="tour__chose_day">
                        <v-date-picker :locale="locale" :popover="{ visibility: 'click' }"  :masks="masks" @dayclick="onDayClick" :minDate="min_date" color="orange" :value="value_in" :attributes='attrs'>
                    ...
                    <div class="return__block">
                        <label class="container">{{translations.one_way}}
                            <input type="radio" @click="oneWay" checked="checked" value="1" name="one_way">
                ...
                <!--Дата обратно-->
                <div class="select__car__date_return ">
                    <!--Вариант обратно-->
                    <div class="return__element_1">
                        <div class="return___radio">
                            <label class="container">
                                <input @click="onReturnEnable" name="one_way" value="0" type="radio">
                                <span class="checkmark"></span>
                                {{translations._return}}
                            </label>
                        </div>
                    </div>
                    <div class="tour__chose_day">
                        <v-date-picker :locale="locale" :popover="{ visibility: 'click' }"  :masks="masks" @dayclick="onDayClickReturn" color="orange" :value="value_out" :minDate="value_in ? min_out_date : min_date" :attributes='attrs'>
                            <template v-slot="{ inputValue, inputEvents }">
                                <div style="position: relative">
                                    <input
                                        ...
                                        v-on="inputEvents"
                                    />
                                    <span v-if="return_disabled" class="disabled-return"></span>
                                </div>
                            </template>
                            <template #day-popover="{ day, attributes }">
                                <div class="text-xs text-gray-300 font-semibold text-center">
...
<script>
    ...
    export default {
        ...
        props: [
            'locations', 'translations', 'currency', 'locale', 'transfer', 'csrf', 'checkout_route'
        ],
        computed: {
            min_out_date() {
        ...
        methods: {
            ...
            setLocation() {
                this.selected_location = document.getElementById('test').value;
                this.recalculate();
            },
            oneWay() {
            ...
            onReturnEnable() {
                let return_date = this.value_in ? this.value_in : null;
                document.getElementById('date-out').value = return_date ? moment(return_date.date).format('DD-MM-YYYY') : '';
                document.getElementById('date-out').dispatchEvent(new Event('change'));
