...
@section('content')
    ...
    @include('backend.partials.title_breadcrumbs',
        ['title' => 'Branches',
         'breadcrumbs' => [
             ['active' => true, 'title' => 'Branches'],
             ],
          'buttons' => [
              'create' => ['title' => 'Create Branch', 'route' => route('backend.branches.create')],
              'reorder' => ['title' => 'Reorder Branch', 'class' => 'info', 'route' => route('backend.branches.reorder')],
              ],
              ])
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    @foreach ($branches as $branch)
                                        <tr>
                                            <td>
                                                <img src="{{ $branch->getFirstMediaUrl('preview') }}" height="45" alt="">
                                            </td>
                                            <td>{{$branch->title}}</td>
                                            <td><input name="active" class="activation"
                                                       data-record-id="{{$branch->id}}"
                                                       data-record-url="{{route('backend.branches.toggle_activate', $branch)}}"
                                                       type="checkbox" {{ $branch->active ? 'checked' : '' }}></td>
                                            <td>
                                            ...
                                            <td >
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                       href="{{route('backend.branches.edit', $branch)}}">Edit</a>
                                                    <div class="dropdown-divider"></div>
                                                    <a class="dropdown-item txt-danger" data-record-action="delete"
                                                       data-record-delete-url="{{route('backend.branches.destroy', $branch)}}"
                                                       data-record-name="{{$branch->title}}"
                                                       data-record-id="{{$branch->id}}" data-bs-toggle="modal"
