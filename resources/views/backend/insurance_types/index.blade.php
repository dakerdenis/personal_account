...
@section('content')
    ...
    @include('backend.partials.title_breadcrumbs',
        ['title' => 'Insure Types',
         'breadcrumbs' => [
             ['active' => true, 'title' => 'Insure Types'],
             ],
          'buttons' => [
              'create' => ['title' => 'Create Insure Type', 'route' => route('backend.insurance_types.create')],
              'reorder' => ['title' => 'Reorder Insure Type', 'class' => 'info', 'route' => route('backend.insurance_types.reorder')],
              ],
              ])
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    @foreach ($insuranceTypes as $insuranceType)
                                        <tr>
                                            <td>{{$insuranceType->title}}</td>
                                            <td><input name="active" class="activation"
                                                       data-record-id="{{$insuranceType->id}}"
                                                       data-record-url="{{route('backend.insurance_types.toggle_activate', $insuranceType)}}"
                                                       type="checkbox" {{ $insuranceType->active ? 'checked' : '' }}></td>
                                            <td>
                                                {{ $insuranceType->created_at->format('d/m/Y H:i:s') }}
                                            </td>
                                            <td >
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                       href="{{route('backend.insurance_types.edit', $insuranceType)}}">Edit</a>
                                                    <div class="dropdown-divider"></div>
                                                    <a class="dropdown-item txt-danger" data-record-action="delete"
                                                       data-record-delete-url="{{route('backend.insurance_types.destroy', $insuranceType)}}"
                                                       data-record-name="{{$insuranceType->title}}"
                                                       data-record-id="{{$insuranceType->id}}" data-bs-toggle="modal"
