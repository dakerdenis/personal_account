...
@section('content')
    ...
    @include('backend.partials.title_breadcrumbs',
        ['title' => 'Departments',
         'breadcrumbs' => [
             ['active' => true, 'title' => 'Departments'],
             ],
          'buttons' => [
              'create' => ['title' => 'Create Department', 'route' => route('backend.departments.create')],
              'reorder' => ['title' => 'Reorder Departments', 'class' => 'info', 'route' => route('backend.departments.reorder')],
              ],
              ])
                                    ...
                                    @foreach ($departments as $department)
                                        <tr>
                                            ...
                                            <td><input name="active" class="activation"
                                                       data-record-id="{{$department->id}}"
                                                       data-record-url="{{route('backend.departments.toggle_activate', $department)}}"
                                                       type="checkbox" {{ $department->active ? 'checked' : '' }}></td>
                                            <td>
                                            ...
                                            <td >
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                       href="{{route('backend.departments.edit', $department)}}">Edit</a>
                                                    <div class="dropdown-divider"></div>
                                                    <a class="dropdown-item txt-danger" data-record-action="delete"
                                                       data-record-delete-url="{{route('backend.departments.destroy', $department)}}"
                                                       data-record-name="{{$department->title}}"
                                                       data-record-id="{{$department->id}}" data-bs-toggle="modal"
